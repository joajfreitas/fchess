...
fn zobryst_hash(board: &Board, turn: &Side) -> u64 {
    ...
    //let offset_piece = 64 * piece_type + 8 * row + col;
    let castle_hash = zobryst_table[768]
        ^ zobryst_table[768 + 1]
        ^ zobryst_table[768 + 2]
        ^ zobryst_table[768 + 3];
    println!("castling: {:x}", castle_hash);
    let enpassant = 0;
    println!("enpassant: {:x}", enpassant);
    let side_hash = match turn {
        Side::White => zobryst_table[780],
        Side::Black => 0,
    };
    println!("side_hash: {:x}", side_hash);
    piece_hash ^ castle_hash ^ side_hash
}
...
fn main() {
    ...
    for i in 0..(bytes.len() / struct_size) {
        ...
        let mov: u16 = ((bytes[i + 8] as u16) << 8) + (bytes[i + 9] as u16);
    ...
    let board =
        Board::read_fen("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1".to_string());

    //let board =
    //Board::read_fen("rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq e3 0 1".to_string());
    println!("zobryst_hash: {:x}", zobryst_hash(&board, &Side::Black));
    println!("{:?}", book[0]);
}
